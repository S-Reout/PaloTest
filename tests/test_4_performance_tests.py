...
@pytest.mark.order3  # works for 5000 as well
def test_get_players_single_thread(num_of_requests=500):
    """
    Test sends many requests to the server (per "num_of_requests").
    ...
    for i in range(num_of_requests):
        responses_list.append(get_players(page_num=1))
    after_timestamp = datetime.now()
